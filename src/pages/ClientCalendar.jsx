...
export default function ClientCalendar({ client, onBack }) {
  ...
  const legend = [
    { label: 'Vídeo', color: '#FF6B00' }, { label: 'Story', color: '#FF8C42' },
    { label: 'Criativo', color: '#555' }, { label: 'Campanha', color: '#FF6B00' },
  ]
      ...
      <div style={{ padding: '16px 28px', borderBottom: '1px solid rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', gap: '16px', background: 'rgba(15,15,15,0.6)', backdropFilter: 'blur(10px)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '11px', color: '#666' }}>{loading ? 'Carregando...' : `${events.length} eventos`}</div>
          </div>
        </div>
        ...
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
          ...
          <Plus size={14} strokeWidth={2.5} /> Novo Evento
        </motion.button>
      </div>

      {/* Calendar */}
      <div style={{ flex: 1, overflow: 'auto', padding: '24px 28px' }}>
        {loading ? <PageLoading message="Carregando eventos..." /> : error ? <PageError message={error} onRetry={refetch} /> : (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} style={{ height: '100%', minHeight: '600px' }}>
            <FullCalendar
              ref={calendarRef}
